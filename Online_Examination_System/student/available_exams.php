...
// UPDATED QUERY: Get all active exams from faculty of the same college
$query = "
...
$stmt->bind_param("issi", $college_id, $now, $now, $student_id);
$stmt->execute();
$result = $stmt->get_result();
...
// Also get exams from Exam_Availability table (original query)
$availability_query = "
    SELECT e.*, f.name as faculty_name
    FROM Exam e
    JOIN Faculty f ON e.created_by = f.faculty_id
...
// Get completed exams
$completed_query = "
    SELECT e.*, ea.attempt_id, ea.start_time as attempt_start, ea.end_time as attempt_end, 
           ea.total_score, ea.status, (ea.total_score / e.total_marks * 100) as percentage
    FROM Exam e
                            ...
                            <div class="card h-100 shadow-sm">
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="d-flex justify-content-between align-items-center">
                                        ...
                                        <?php
                                        $now_dt = new DateTime();
                                        $start_dt = new DateTime($exam['start_time']);
                                        $end_dt = new DateTime($exam['end_time']);
                                        if ($now_dt >= $start_dt && $now_dt <= $end_dt):
                                        ?>
                                            <a href="take_exam.php?exam_id=<?php echo $exam['exam_id']; ?>" class="btn btn-success">
                                                <i class="fas fa-pen-alt me-2"></i>Take Exam
                                            </a>
                                        <?php elseif ($now_dt < $start_dt): ?>
